...
func (a *Adapter) Inspect(ctx context.Context, resource model.Resource) (model.Observation, error) {
	...
	if installed {
		for _, command := range resource.Commands {
			...
			if _, evalErr := a.fs.EvalSymlinks(path); evalErr != nil {
				observation.Healthy = false
...
func (a *Adapter) Simulate(ctx context.Context, operation model.Operation) (provider.ChangeSet, error) {
	...
	case model.OperationUpgrade:
		...
		outdated, err := parseOutdated(result.Stdout, a.kind, operation.Package)
		if err != nil {
...
func (a *Adapter) installCaskWithAdoption(ctx context.Context, operation model.Operation, resource model.Resource, app appDeclaration) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			_, runErr := a.run(ctx, "install", "--cask", operation.Package)
			return runErr
		}
		return err
	}
	...
	restore := func(cause error) error {
		if _, statErr := a.fs.Lstat(app.path); statErr == nil {
			...
			if _, failedErr := a.fs.Lstat(failed); failedErr == nil || !errors.Is(failedErr, os.ErrNotExist) {
				return errors.Join(cause, fmt.Errorf("homebrew: refusing to overwrite failed recovery artifact %q", failed))
			}
			if moveErr := a.fs.Rename(app.path, failed); moveErr != nil {
				return errors.Join(cause, fmt.Errorf("homebrew: stage failed replacement: %w", moveErr))
			}
		} else if !errors.Is(statErr, os.ErrNotExist) {
			return errors.Join(cause, fmt.Errorf("homebrew: inspect failed replacement destination: %w", statErr))
		}
		if restoreErr := a.fs.Rename(backup, app.path); restoreErr != nil {
			return errors.Join(cause, fmt.Errorf("homebrew: restore original app: %w", restoreErr))
		}
		return cause
	}
...
func parseInfo(contents []byte, kind Kind, packageName string) (bool, string, error) {
	...
	if err := decodeOne(contents, &payload); err != nil {
		return false, "", err
	}
	if kind == Formula {
		if len(payload.Formulae) != 1 || len(payload.Casks) != 0 {
			return false, "", errors.New("expected exactly one formula and no casks")
		}
		item := payload.Formulae[0]
	...
	if len(payload.Casks) != 1 || len(payload.Formulae) != 0 {
		return false, "", errors.New("expected exactly one cask and no formulae")
	}
	item := payload.Casks[0]
...
func parseListVersions(contents []byte, packageName string) (string, error) {
	...
	if len(fields) != 2 || !matchesPackage(fields[0], fields[0], packageName) || fields[1] == "" {
		return "", fmt.Errorf("unexpected list output %q", line)
	}
	return fields[1], nil
}

func parseOutdated(contents []byte, kind Kind, packageName string) (bool, error) {
	var payload struct {
		Formulae []struct {
			Name     string `json:"name"`
			FullName string `json:"full_name"`
	...
	if kind == Formula {
		if len(payload.Casks) != 0 || len(payload.Formulae) > 1 {
			return false, errors.New("outdated response contains unexpected packages")
		}
		if len(payload.Formulae) == 0 {
			return false, nil
		}
		if !matchesPackage(payload.Formulae[0].Name, payload.Formulae[0].FullName, packageName) {
			return false, errors.New("outdated formula package mismatch")
		}
		return true, nil
	}
	if len(payload.Formulae) != 0 || len(payload.Casks) > 1 {
		return false, errors.New("outdated response contains unexpected packages")
	}
	if len(payload.Casks) == 0 {
		return false, nil
	}
	if !matchesPackage(payload.Casks[0].Token, payload.Casks[0].FullToken, packageName) {
...
func matchesPackage(short, full, packageName string) bool {
	...
	if slash := strings.LastIndexByte(packageName, '/'); slash >= 0 {
		base = packageName[slash+1:]
	}
	return full == packageName || short == packageName || short == base && (full == "" || full == base)
}
