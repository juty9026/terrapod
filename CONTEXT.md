...
- `install.sh` reads the managed config at the default chezmoi path and does not honour `TERRAPOD_CHEZMOI_CONFIG`, because it runs chezmoi without `--config`; `tpod` honours the override because it passes it on to chezmoi.
...
- First-run resume additionally reruns **Terrapod Setup** when the stored profile differs from the detected one; that match is a first-run rule layered on completeness, and `tpod` does not compare them.
...
- The recovery core includes the shared managed config reader at `~/.local/lib/terrapod/config-toml.sh`, because the installed `tpod` command reads the managed config before it delegates to chezmoi; the rest of `~/.local/lib/terrapod/` still arrives with the full declared-state apply.
- First-run recovery-core validation requires the installed `terrapod` executable, the installed `tpod` alias, the installed managed config reader, and a successful `~/.local/bin/tpod help` invocation; file presence alone is not enough to mark the installer recoverable.
...
- The marker-and-success contract reserves a non-zero installer exit for the case where the Terrapod install warning marker itself cannot be written, because an unrecorded failure would otherwise disappear from recovery visibility.
- A failed attempt to clear a Terrapod install warning marker does not fail the installer script; the stale marker stays visible in `tpod doctor` until a later successful rerun clears it.
- A `run_onchange` installer that exits successfully after recording a Terrapod install warning marker needs a companion `run_before` retry script, because chezmoi records the script state on success and would not rerun the failed category on the next apply.
...
- Jetendard is pinned to the installed release until the managed font installer source changes; the supported way to move to a newer upstream release is running `~/.local/lib/terrapod/jetendard-font install` directly, and removing the manifest is not an upgrade path because nothing reinstalls the font.
