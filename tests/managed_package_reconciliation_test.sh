...
printf '%s\n' "claude|user|official-uninstaller" >"$inventory/vendor"
plan="$(TERRAPOD_MANAGED_PACKAGE_INVENTORY_DIR="$inventory" "$managed_packages" plan "$config" macos-terminal)"
assert_contains "$plan" "remove|claude|vendor|claude|claude uninstall" "planner prefers a verified official vendor uninstaller"
