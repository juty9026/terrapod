...
- 보호된 OS prerequisite, system runtime, shared install, package-manager state
...
`terrapod update`는 `chezmoi update --exclude scripts`로 Terrapod Source Repository와 installed command를 새로고침한 뒤, 갱신된 `~/.local/bin/tpod apply` process로 handoff합니다. 광범위한 Homebrew, APT, mise upgrade는 실행하지 않습니다.
...
`tpod status`는 declared package installation과 primary executable selection을 보여주는 snapshot입니다.
`tpod doctor`는 readiness gate입니다. declared provider package나 canonical executable이 없거나,
enabled command를 PATH에서 사용할 수 없거나, 다른 enabled requirement 또는 install warning이
남아 있으면 non-zero로 종료합니다. canonical executable이 존재하지만 다른 executable이 먼저
선택되면 actual path와 canonical path를 advisory로 표시하고 실패하지 않습니다.
...
Jetendard는 managed font installer source가 바뀌기 전까지 Terrapod이 설치한 release에 고정됩니다. 상위 release가 새로 나와도 업그레이드하지 않고, `tpod apply`를 다시 실행해도 마찬가지입니다. install script는 rendered content가 바뀔 때만 다시 실행되고, retry script는 `jetendard-font` install warning이 기록되어 있을 때만 실행되기 때문입니다. 최신 안정 release로 직접 올리려면 installer helper를 실행합니다.

```sh
~/.local/lib/terrapod/jetendard-font install
```

helper는 최신 안정 release의 모든 TTF를 설치하고, 이전 release가 소유하던 파일을 제거하며, `tpod status`와 `tpod doctor`가 읽는 manifest를 다시 씁니다. 실행한 뒤에는 Ghostty, Zed 또는 Orca를 다시 시작합니다. manifest를 지우는 것은 업그레이드 경로가 아닙니다. font를 다시 설치하지 않으며, helper를 실행하기 전까지 두 검사 모두 Jetendard가 없다고 보고합니다.
...
package를 복원하며 version upgrade나 alternate installation 제거를 실행하지 않습니다.
설치 뒤에는 활성화된 command-bearing declaration의 canonical executable path를 확인하고
다른 executable이 primary이면 selection advisory를 표시합니다.
...
Terrapod은 기존 mise, APT, vendor-installed 및 기타 alternate payload를 자동으로 제거하지 않습니다.
활성화된 declaration이 canonical provider를 통해 설치되었는지와 어떤 executable이 먼저 선택되는지만
확인합니다. 선택된 executable의 installer provenance를 추론하거나 uninstall command를 제안하지 않습니다.
