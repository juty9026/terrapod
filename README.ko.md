...
`tpod status`는 canonical installation, primary executable, pending migration을 빠르게 보여주는 snapshot입니다.
`tpod doctor`는 full read-only Managed Package scan을 수행하는 readiness gate입니다.
canonical package가 없거나 managed default PATH에서 legacy executable이 primary인 경우,
...
package를 복원하며 version upgrade를 실행하지 않습니다. canonical installation 뒤에는
exact registry identity를 deep scan하고 안전한 legacy payload removal을 한 목록으로 보여준 뒤
`Proceed with removing these legacy package installations? [y/N]`로 한 번만 묻습니다.
거절, non-interactive 실행, 실패, 보호 대상, shared ownership, unresolved action은 warning으로
남고 apply 자체를 실패시키지 않습니다.
...
Terrapod은 exact registry match와 안전한 ownership이 검증된 payload만 migrate합니다.
canonical mise runtime version과 project-local selection은 보존합니다. APT removal에는
manual-install state와 no-cascade simulation이 필요합니다. protected prerequisite,
shared/root-owned install, nonstandard Homebrew prefix, unknown PATH copy는 manual action으로
남습니다. 기존 app artifact에는 Homebrew Cask adoption을 사용하며, adoption failure는
manual action이고 app bundle을 Trash로 옮기지 않습니다.

`~/.config/zsh/path.d`의 file은 명시적인 machine-local PATH override입니다. 항상 가장 높은
우선순위를 유지하고 reconciliation이 제거하지 않으며, Terrapod은 canonical package를
설치한 상태에서 advisory로만 보고합니다.
