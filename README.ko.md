...
Homebrew로 `chezmoi`와 `gum`을 설치합니다. 이어서 최신 Stable Release를
설치하고 그 release에서 Setup을 실행합니다. setup이 성공한 다음에만 initial
declared-state apply를 실행합니다.
authoring checkout은 active Stable Release와 분리되어 있어 checkout을 편집해도 Stable Release를 설치하기 전에는 active manager가 바뀌지 않습니다.
initial apply가 끝나면 installer는 `tpod help`를
출력해서 짧은 day-to-day command를 바로 볼 수 있게 합니다.
...
pre-manager 구현을 쓰던 maintainer machine에서는 먼저 `tpod update`를 한 번 실행하고 출력된 안내를 따른 뒤, 다시 한 번 실행합니다.
두 번째 호출이 one-shot manager transition을 자동으로 수행합니다.
legacy chezmoi data를
`~/.config/terrapod/config.json`으로 변환하고, 이관 가능한 기존 resource의
Terrapod ownership을 가져온 뒤 reconcile하며, 검증이 끝난 경우에만 legacy
source를 제거합니다. transition은 `install.sh --migrate`를 internal recovery
primitive으로 사용하며, 신규 설치와 routine manager update는 이 migration을
실행하지 않습니다.
...
실패하면 installer는 initial apply 전에 멈추고, Stable Release installer를 재개하는
정확한 command를 출력합니다.
...
bootstrap 이후에는 routine management와 Stable Release update에 `tpod`를 사용합니다.
...
Terrapod은 canonical GitHub repository가 HTTPS로 제공하는 Stable Release metadata를 검증하고,
activation 전에 모든 asset의 size와 SHA-256 digest를 확인합니다. `tpod update`는
이후 전체 plan을 출력하고 새 Management Core를 atomic하게 활성화한 뒤
Terrapod-owned resource를 reconcile합니다. Terrapod ownership state 밖의 package는
upgrade하거나 remove하지 않습니다.

Stable GitHub Release는 macOS와 Linux의 `amd64`, `arm64`용 static `tpod` binary
4개, immutable source archive, release-bound Resource Catalog, `release.json`,
versioned `install.sh`로 구성된 정확히 8개의 asset을 포함합니다.
...
release-bound source, independent config, destination, script exclusion을 고정하며,
...
정확한 version의 `install.sh --repair`를 실행합니다. repair는 동일한 Stable
...
release-bound Resource Catalog는 양쪽 profile의 mandatory CLI formula 20개를 선언합니다.
...
Machine-specific Homebrew package는 active Resource Catalog가 선언하지 않는 한 Terrapod
밖에 둡니다.
...
unavailable이면 launcher가 출력한 정확한 `install.sh --repair` command를
사용합니다.

### Intentional Upgrades

`tpod apply`는 active Resource Catalog를 reconcile합니다. 누락된 declared
...
Routine command는 active Resource Catalog의 schema에 따라 config를 검증하고 새
