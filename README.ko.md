...
declared-state apply를 실행합니다. initial apply가 끝나면 installer는 `tpod help`를
...
- 해당 stack이 활성화된 경우에만 설치하는 Linux Homebrew와 Optional AI Tool Stack cask
...
Homebrew와 APT는 platform bootstrap state를 준비합니다. Homebrew는 세 개의 cross-profile Optional AI Tool Stack cask도 담당합니다. mise는 나머지 shared command-line tool과 development runtime을 위한 Modern CLI Provider입니다.
...
Homebrew가 관리하는 AI CLI만 양쪽 profile에서 명시적으로 upgrade합니다.
...
Terrapod은 기존 vendor-installed AI CLI binary를 제거하지 않습니다. 해당 binary가 Homebrew-managed command를 가리면 `tpod status`와 `tpod doctor`가 수동 정리 guidance를 제공합니다.
